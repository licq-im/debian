...
[
  ...
  AC_CACHE_VAL(ac_cv_lib_qtlib, [
      ...
      if test -n "$QTDIR"; then
        qt_library_dirs="$QTDIR/lib64 $QTDIR/lib $qt_library_dirs"
      fi
...
[
  ...
  AC_CACHE_VAL(ac_cv_header_qtinc, [
    ...
    if test -z "$ac_qt_includes"; then
      ...
      if test -n "$QTDIR"; then
        qt_include_dirs="$QTDIR/include $QTDIR/include/qt $QTDIR/include/qt3 $qt_include_dirs"
      fi
...
[
  ...
  if test "$ac_cv_qt_setup" != "yes"; then
    AC_MSG_ERROR([
      Sorry, but you need Qt version 3.x to compile the Qt gui plugin.
    ])
  fi
])

AC_DEFUN([AC_PATH_QT_LUPDATE],
[
  AC_PATH_PROG(
    QT_LUPDATE,
    lupdate,
  ...
  if test "$QT_LUPDATE" = "echo"; then
    echo "** lupdate could not be found. You will not be able to update the translations."
  fi

  AC_SUBST(QT_LUPDATE)
])

AC_DEFUN([AC_PATH_QT_LRELEASE],
[
  AC_PATH_PROG(
    QT_LRELEASE,
    lrelease,
...
[
  if test "$have_kde" = "yes"; then
    AC_PATH_PROG(
      ...
      kde-config,
      no,
      /opt/kde3/bin:/opt/kde/bin:/usr/lib/kde/bin:/usr/local/kde/bin/:/usr/kde/bin:/usr/lib/kde/bin:/usr/bin:/usr/X11R6/bin:$PATH)

    if test "$KDE_CONFIG" = "no"; then
      AC_MSG_ERROR(["Could not find kde-config, make sure it's in your PATH"])
    fi
  fi
...
[
  if test "$have_kde" == "yes"; then
    ...
    if test "$DCOPIDL" = "echo" || test "$DCOPIDL2CPP" = "echo"; then
      AC_MSG_ERROR(["DCOP must be installed. Could not find dcopidl or dcopidl2cpp"])
    fi
  fi
...
[
  ...
  if test -n "$ac_kde"; then
    kde_incdirs="$ac_kde/include $ac_kde $kde_incdirs"
  fi
...
[
  ...
  if test -n "$ac_kde"; then
    kde_libdirs="$ac_kde/lib $ac_kde $kde_libdirs"
  fi
...
AC_DEFUN([LICQ_CHECK_QT_HEADER],
[
  AC_MSG_CHECKING([for specific Qt header files])

  LICQ_FIND_FILE(qinterlacestyle.h, $ac_cv_header_qtinc, qt_incdir)

  if test "$qt_incdir" != ""; then
    AC_DEFINE_UNQUOTED(HAVE_QINTERLACESTYLE_H, 1, [Undefine if qinterlacestyle.h is not present on your Qt installation])
  fi
...
dnl Check if we should use -Wextra or -W to get extra warnings
AC_DEFUN([LICQ_CXX_WEXTRA],
[
...
[ AC_REQUIRE([_AM_PATH_GPGME_CONFIG])dnl
  ...
  if test $ok = yes; then
    ...
    AC_TRY_LINK([
      ...
      gpgme_data_t data;
      if (gpgme_data_new(&data) == GPG_ERR_NO_ERROR)
...
[
AC_ARG_WITH([boost],
  AS_HELP_STRING([--with-boost-dir@<:@=DIR@:>@], [specify boost directory (optional)]),
  [
...
*-*-beos* | *-*-cygwin* | *-*-pw32* | *-*-darwin*)
  ...
  ;;
*-ncr-sysv4.3*)
  AC_SEARCH_LIBS(_mwvalidcheckl, mw)
